...
class PCA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        aligned_relative_position_index = aligned_relative_position_index.reshape\
            ...
            .permute(0,2,1,3,4).reshape(4*self.permuted_window_size[0]*self.permuted_window_size[1],self.permuted_window_size[0]*self.permuted_window_size[1])
    ...
    def forward(self, qkv, h, w, mask=None):
        ...
        B, _, C = q.shape
        q = window_partition(q.view(B, h, w, C), self.window_size[0]).view(-1, self.window_size[0] * self.window_size[0], C)
        k = window_partition(k.view(B, h, w, C), self.window_size[0]).view(-1, self.window_size[0] * self.window_size[0], C)
        b_, n, c = q.shape

        kv = self.kv(k).reshape(b_,self.permuted_window_size[0],2,self.permuted_window_size[1],2,2,c//4).permute(0,1,3,5,2,4,6).reshape(b_, n//4, 2, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        q = q.reshape(b_, n, 1, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        q = q * self.scale
...
class PCA_Block(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        h, w = x_size
        permuted_window_mask = torch.zeros((1, h // 2, w // 2, 1))  
    ...
    def forward(self, x, Ref, x_size):
        ...
        img = self.after_norm1(img)
        img_Ref = self.after_norm1(img_Ref)

        qkv = self.qkv(img).reshape(b, -1, 3, c).permute(2, 0, 1, 3)
        qkv_Ref = self.qkv(img_Ref).reshape(b, -1, 3, c).permute(2, 0, 1, 3)

        q = qkv[0].transpose(-2, -1).contiguous().view(b, c, h, w)
        qkv[1, :, :, :] = qkv_Ref[1]
        qkv[2, :, :, :] = qkv_Ref[2]
        ...
        if self.shift_size > 0:
            ...
            shifted_qkv = torch.roll(qkv, shifts=(-self.shift_size, -self.shift_size), dims=(2, 3))
            shifted_qkv = shifted_qkv.view(3, b, l, c)

            if self.input_resolution != x_size:
                attn_windows = self.attn(shifted_qkv, h, w, mask=self.calculate_mask(x_size).to(x.device))
            else:
                attn_windows = self.attn(shifted_qkv, h, w, mask=self.attn_mask)

            x1 = torch.roll(attn_windows, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
            attened_x = x1
        else:
            shifted_qkv = qkv
            attn_windows = self.attn(shifted_qkv, h, w).view(b, l, c).contiguous()
            attened_x = attn_windows

        # merge windows
        attn_windows = attened_x.view(-1, self.window_size, self.window_size, c)
...
class PCA_Group(nn.Module):
    ...
    def __init__(self,
        ...
        self.before_PCA_Group_conv = emptyModule()
        self.after_PCA_Group_conv = emptyModule()
        self.after_PCA_Group_Residual = emptyModule()

    def forward(self, x, Ref, x_size):
        return self.after_PCA_Group_Residual(self.after_PCA_Group_conv(self.patch_embed(self.conv(self.patch_unembed(self.before_PCA_Group_conv(self.residual_group(x, Ref, x_size)), x_size)))) + x)
...
class UPCAmple(nn.Sequential):
    ...
    def __init__(self, scale, num_feat):
        ...
        super(UPCAmple, self).__init__(*m)


class UPCAmpleOneStep(nn.Sequential):
    """
    Args:
        scale (int): Scale factor. Supported scales: 2^n and 3.
        num_feat (int): Channel number of intermediate features.
    ...
    def __init__(self, scale, num_feat, num_out_ch, input_resolution=None):
        ...
        super(UPCAmpleOneStep, self).__init__(*m)
...
class PCAT(nn.Module):
    ...
    Args:
        ...
        uPCAmpler: The reconstruction reconstruction module. 'pixelshuffle'/'pixelshuffledirect'/'nearest+conv'/None
    ...
    def __init__(self,
        ...
        #Permuted Cross-Attention Group  (PCAG)
        ...
        if self.uPCAmpler == 'pixelshuffle':
            self.conv_before_uPCAmple = nn.Sequential(
                nn.Conv2d(embed_dim * 2, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
            self.uPCAmple = UPCAmple(upscale, num_feat)
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
        elif self.uPCAmpler == 'pixelshuffledirect':
            self.uPCAmple = UPCAmpleOneStep(upscale, embed_dim, num_out_ch,
                                            (patches_resolution[0], patches_resolution[1]))
        elif self.uPCAmpler == 'nearest+conv':
    ...
    def forward(self, x, Ref):
        ...
        fused = torch.cat([out_x, out_ref], dim=1)

        out = self.conv_before_uPCAmple(fused)
        return [out]
    ...
    def flops(self):
        ...
        flops += self.uPCAmple.flops()
        return flops

@register('pcat')
def make_pcat(no_uPCAmpling=True):
    return PCAT(upscale=1,
               img_size=64,
               window_size=32,
               depth=[6, 6, 6, 6], 
               embed_dim=180,
